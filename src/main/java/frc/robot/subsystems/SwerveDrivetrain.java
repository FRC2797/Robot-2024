  ...
  public Command driveToFacePoint(Translation2d pointToFace) {
    return defer(
      () -> {
        ...
        Translation2d difference = currentPose.getTranslation().minus(pointToFace);
        double distance = currentPose.getTranslation().getDistance(pointToFace);
        
        double calculatedAngle = Math.acos(difference.getY() / distance);
        double rotationNeeded = Units.degreesToRadians(-90) + calculatedAngle;
        // Should calculate the least rotation
        return driveToRotation(rotationNeeded);
      }
    );
  }
